...
test.describe('Cadena de ADN 3D', () => {
  test.beforeEach(async ({ page }) => {
    await page.addInitScript(() => localStorage.removeItem('educonect:module:cie-408'));
  });

  test('carga offline, identifica estructuras y completa una misión', async ({ page }) => {
    const external = [];
    page.on('request', request => {
      if (ORIGIN_POLICY.isExternalRequest(request.url())) external.push(request.url());
    });

    await page.goto('/modulos/cie-408/');
    await expect.poll(() => page.evaluate(() => window.adn3dReady)).toBe(true);
    await expect(page.locator('#scene canvas')).toBeVisible();

    await page.locator('button[data-part="esqueleto"]').click();
    await expect(page.locator('#partName')).toHaveText('Esqueleto');
    await expect(page.locator('#missionStatus')).toContainText('Misión cumplida');

    await page.locator('button[data-part="base"]').click();
    await expect(page.locator('#partName')).toHaveText('Bases nitrogenadas');
  ...
  test('la API publica el módulo con metadatos', async ({ request }) => {
    ...
    const module = data.modulos.find(item => item.id === 'cie-408');
    expect(module).toBeTruthy();
    expect(module.categoria).toBe('Ciencias');
    expect(module.icono).toBe('🧬');
    expect(module.path).toBe('/modulos/cie-408/');
  });
});
